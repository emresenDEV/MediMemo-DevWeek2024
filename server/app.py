...
def client_signup():
    ...
    try:
        new_client = Client(email=form_data["email"])
        ...
        # if the inputed provider code is the expected length
        ...
        response = make_response({"ERROR": "Could not create client"}, 400)
    return response
...
def clients():
    ...
    if request.method == "GET":
        ...
        response = make_response(client_dict, 200)
...
def clients_by_id(id):
    ...
    if client:
        if request.method == "GET":
            response = make_response(client.to_dict(), 200)
...
def providers():
    ...
    if request.method == "GET":
        ...
        response = make_response(provider_dict, 200)
    elif request.method == "POST":
        form_data = request.get_json()
        try:
...
def providers_by_id(id):
    ...
    if provider:
        if request.method == "GET":
            response = make_response(provider.to_dict(), 200)
        elif request.method == "PATCH":
            form_data = request.get_json()
            try:
                for attr in form_data:
...
def clients_providers():
    ...
    if request.method == "GET":
        ...
        client_provider_dict = [
            client_provider.to_dict() for client_provider in clients_providers
        ]
        response = make_response(client_provider_dict, 200)
    elif request.method == "POST":
        form_data = request.get_json()
        try:
...
def clients_providers_by_id(id):
    ...
    if client_provider:
        ...
        response = make_response({"error": "ClientProvider not found"}, 404)
    return response
