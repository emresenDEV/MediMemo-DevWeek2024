...
emails = []
possible_allergens = ['Peanuts', 'Penicillin', 'NSAIDs', 'Iodinated contrast', 'Gadolinium contrast', 'Ester local anesthetics', 'Latex',
                      'Soy', 'Dairy', 'Eggs', 'Shellfish', 'Gluten', 'Fish', 'Tree nuts', 'Wheat', 'Sesame', 'Sulfites', 'Corn', 'Kiwi', 'Mango', 'Pineapple']
possible_reactions = ['Anaphylaxis', 'Hives', 'Rash', 'Itching', 'Swelling', 'Coughing', 'Wheezing', 'Shortness of breath', 'Nausea', 'Vomiting',
                      'Diarrhea', 'Abdominal pain', 'Dizziness', 'Fainting', 'Low blood pressure', 'Fast heart rate', 'Confusion', 'Loss of consciousness']
possible_vaccines = ['COVID-19', 'Influenza', 'Hepatitis A', 'Hepatitis B', 'HPV', 'Measles, Mumps, Rubella',
                     'Pneumococcal', 'Polio', 'Tetanus, Diphtheria, Pertussis', 'Varicella', 'Zoster']
possible_surgeries = ['Appendectomy', 'Cholecystectomy', 'Hernia repair', 'Mastectomy', 'Tonsillectomy', 'Adenoidectomy', 'Cataract surgery', 'Coronary artery bypass',
                      'Heart valve repair', 'Hip replacement', 'Knee replacement', 'Spinal fusion', 'Laminectomy', 'Lobectomy', 'Lung transplant', 'Wisdom Teeth Removal', 'ACL repair', 'Rotator cuff repair']
possible_conditions = ['Heart disease', 'Stroke', 'Diabetes', 'Cancer', 'High blood pressure', 'High cholesterol', 'Asthma', 'Arthritis', 'Osteoporosis', 'Alzheimer\'s', 'Dementia', 'Depression', 'Anxiety', 'Bipolar disorder', 'Schizophrenia', 'ADHD', 'Autism',
                       'Epilepsy', 'Migraines', 'Thyroid disease', 'Kidney disease', 'Liver disease', 'Lung disease', 'COPD', 'Crohn\'s disease', 'Ulcerative colitis', 'Celiac disease', 'Multiple sclerosis', 'Lupus', 'Rheumatoid arthritis', 'Psoriasis', 'HIV/AIDS', 'Hepatitis', 'Tuberculosis']
medical_conditions = ['Asthma', 'Diabetes', 'High blood pressure', 'High cholesterol', 'Heart disease', 'Stroke', 'Cancer', 'Arthritis', 'Osteoporosis', 'Alzheimer\'s', 'Dementia', 'Depression', 'Anxiety', 'Bipolar disorder', 'Schizophrenia', 'ADHD', 'Autism', 'Epilepsy',
                      'Migraines', 'Thyroid disease', 'Kidney disease', 'Liver disease', 'Lung disease', 'COPD', 'Crohn\'s disease', 'Ulcerative colitis', 'Celiac disease', 'Multiple sclerosis', 'Lupus', 'Rheumatoid arthritis', 'Psoriasis', 'HIV/AIDS', 'Hepatitis', 'Tuberculosis']
...
def generate_patient_data():
    ...
    religion = fake.random_element(elements=(
        'Christian', 'Muslim', 'Jewish', 'Buddhist', 'Hindu', 'Atheist', 'Agnostic', 'Other'))
    eyesight = fake.random_element(
        elements=('20/20', '20/40', '20/60', '20/80', '20/100', 'blind'))
    hearing = fake.random_element(
        elements=('hearing', 'hard of hearing', 'deaf'))
    ...
    if is_nonbinary:
        gender = 'nonbinary'
        pronouns = fake.random_element(elements=('they/them', 'she/her', 'he/him', 'she/they', 'he/they',
                                       'she/they/he', 'he/they/she', 'they/she/he', 'they/he/she', 'they/she', 'they/he', 'she/he'))
    ...
    if sex == 'FtM male' or sex == 'female' or sex == 'intersex':
        ...
        if is_pregnant:
            last_menstrual = fake.date_between(
                start_date='-280d', end_date='today')
        else:
            last_menstrual = fake.date_between(
                start_date='-28d', end_date='today')
    # if male anatomy
    else:
        is_pregnant = False
        last_menstrual = 'NULL'
    ...
    for vaccine in their_vaccines:
        vaccines.update({vaccine: str(fake.date_between(
            start_date='-'+str(age)+'y', end_date='today'))})
    ...
    for surgery in their_surgeries:
        surgeries.update({surgery: str(fake.date_between(
    ...
    while len(legal_guardians) < num_guardians:
        ...
        email = first_name[0].lower() + last_name.lower() + \
            '@' + fake.domain_name()
        phone = fake.random_int(min=1000000000, max=9999999999)
        legal_guardians.append({'first_name': first_name, 'last_name': last_name,
                               'relationship': relationship, 'phones': [phone], 'emails': [email]})
...
headers = ['email', 'password', 'name_title', 'first_name', 'middle_name', 'last_name', 'name_suffix', 'date_of_birth', 'sex', 'gender', 'pronouns', 'religion', 'eyesight', 'hearing', 'mobility', 'is_pregnant', 'last_menstrual', 'preferred_name', 'SSN',
           'height', 'weight', 'vaccines', 'allergies', 'surgeries', 'family_history', 'last_reviewed_timestamp', 'emergency_contacts', 'last_reviewer_staff_id', 'blood_type', 'phone', 'medical_history', 'pharmacy_name', 'pharmacy_address', 'pharmacy_phone']
